...
class TTSAPI:
    def __init__(self):
        ...
        if self.api_key:
            print(f"✓ ElevenLabs API key found: {self.api_key[:8]}...{self.api_key[-4:]}")
            ...
            print("   Add it to your .env file to enable text-to-speech")
    ...
    def text_to_speech(self, text: str, voice_id: str = None, model_id: str = None, output_format: str = None):
        if not self.client:
            raise ValueError("ElevenLabs client not initialized - API key missing")
